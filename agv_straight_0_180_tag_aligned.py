...
MAX_TAG_HEADING_OFFSET_DEG = 3.0  # logging/outlier reference only; map target is never shifted
...
def heading_error_from_tag(map_heading, tag_heading):
    """Return the shortest signed error from the tag heading to the map heading.

    The map target remains fixed at 0 or 180 degrees.  The tag heading is sent
    separately to the ESP32, where it realigns the IMU estimate.
    """
    ...
    try:
        while True:
            ...
            if key == ord("s") and mode == MODE_DOCK_WAIT:
                ...
                dock_tag_error = heading_error_from_tag(
            ...
            if mode == MODE_DOCK_TO_TAG1:
                ...
                if reached_y_centre:
                    send_velocity(ser, 0.0, 0.0, 0.0, 0.0)
