...
class Config:
    ...
    # Serial (matches your ESP32 firmware)
...
class AGVLogger:
    ...
    def __init__(self):
        ...
        self.display_buffer: List[Tuple[str, str, str]] = []  # (level, subsystem, msg)
...
class DisplayEngine:
    ...
    def __init__(self, logger: AGVLogger):
        ...
        self._running = True
        self._view = "splash"  # splash | boot | dashboard | emergency | shutdown
    ...
    def _draw_splash(self):
        ...
        self._centered_text("AGV-OS v2.1.0  |  ISO 3691-4  |  VDA 5050",
...
class HardwareManager:
    ...
    def detect_serial(self):
        ...
        for p in ports:
            self.log.info('HW', f"  {p.device}: {p.description}", 'HW')

        # Check expected port
        if Path(Config.SERIAL_PORT).exists():
            self.tests['serial_port'] = True
            self.log.info('HW', f"Port {Config.SERIAL_PORT} ready", 'HW')
            return True

        # Auto-detect
        for p in ports:
            desc = p.description.lower()
            if any(x in desc for x in ['usb', 'uart', 'cp210', 'ch340', 'ftdi']):
                Config.SERIAL_PORT = p.device
                self.tests['serial_port'] = True
                self.log.info('HW', f"Auto-detected ESP32 on {p.device}", 'HW')
                return True

        self.log.error('HW', "ESP32 not found on any port", 'HW')
        return False
...
class BootSequence:
    ...
    def run(self):
        ...
        self.log.info('INIT', "AGV-OS v2.1.0 starting", 'INIT')
        ...
        if all_pass:
            ...
            self.log.error('READY', "BOOT FAILED - Critical errors detected", 'EMERGENCY')
            return False
...
class AGVOS:
    ...
    def run(self):
        ...
        if not self._boot_complete:
            self.log.error('SYS', "Boot failed. System halted.", 'EMERGENCY')
            time.sleep(5.0)
            self.display.stop()
            return 1
    ...
    def _operational_loop(self):
        ...
        while self._running:
            ...
            # Check for key input (non-blocking via OpenCV)
            if CV2_AVAILABLE:
